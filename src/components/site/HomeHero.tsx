...
export default function HomeHero() {
  ...
  const override = content.featuredVideo;
  const catalogVideo = messages[0];
  const featured = override
    ? {
        title: override.title,
        description: override.description,
        date: override.date,
        dateTime: override.date || undefined,
        image: `https://i.ytimg.com/vi/${override.youtubeId}/hqdefault.jpg`,
        imageAlt: override.title,
        href: `https://www.youtube.com/watch?v=${override.youtubeId}`,
        external: true,
      }
    : {
        title: catalogVideo.title,
        description: catalogVideo.description,
        date: catalogVideo.date ?? "",
        dateTime: catalogVideo.dateTime,
        image: catalogVideo.image,
        imageAlt: catalogVideo.imageAlt,
        href: `/mensagens/${catalogVideo.slug}`,
        external: false,
      };
  const external = featured.external ? { target: "_blank", rel: "noopener noreferrer" } : {};
  return <section className="home-hero editorial-hero" aria-labelledby="welcome-title">
    <div className="content-width editorial-hero-grid">
      ...
      <article className="hero-message"><a className="hero-message-image" href={featured.href} aria-label={`Assistir ${featured.title}`} {...external}><img src={featured.image} alt={featured.imageAlt} width="720" height="405" decoding="async" /><span className="play-symbol"><Play aria-hidden="true" /></span><span className="hero-watch-label">{override ? "Vídeo em destaque" : "Mensagem mais recente"}</span></a>
        <div className="hero-message-caption">{featured.date ? <time dateTime={featured.dateTime}>{featured.date}</time> : null}<h2><a href={featured.href} {...external}>{featured.title}</a></h2><p>{featured.description}</p></div>
      </article>
    </div>
